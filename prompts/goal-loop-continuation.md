...
**State: ACTIVE — not yet auditor-approved.** Prose closes nothing: saying "done", "complete", or "shipped" in plain text does NOT close this goal — the session just continues. The ONLY way to close it is a `complete_goal` tool call that survives the isolated auditor. If the work is genuinely complete, call `complete_goal` NOW instead of narrating completion; if blocked, call `pause_goal` with the blocker. A done-but-unclosed goal is a bug, not a resting state. If the goal is PAUSED and the user has authorized continuation in this conversation (answered the decision, waived the blocker, supplied the missing input), call `resume_goal` yourself and keep working — do not bounce the user to `/goal resume` for an authorization they just gave you. `complete_goal` only runs on an active item, so resume first, then claim.
...
The agent-written completionSummary is user-facing evidence, not a step log: open with the outcome in plain words, then one durable result per label. The posted chat is a human account of what happened: lead with the behavior/code changes and remaining limitations. Tests, verification, gate tables, commands, hashes, and machine evidence are archive-only by default; the user can explicitly ask for a compact verification sentence, in which case pass `showVerification: true` to complete_goal. The archive always retains the full commands, statuses, hashes, and machine evidence. Keep each label value to one or two short clauses — findings/values render uncapped (a 10k-char guard stops only pathological megabytes), so write full values, not clipped stubs (Next keeps the one-concrete-action rule — see below); cite human-readable proof (counts, versions, repo-relative paths, PR numbers) and never machine paths (sidecar/log/tarball paths under /tmp or /var/tmp — those live in the archive only; gate repro commands, commit hashes and repository bookkeeping stay in the durable archive, not chat). Cut values at clause boundaries, never mid-word; write Next: as one concrete follow-up action or none (audit-self-referential lines like "verdict decides" are stripped at render); pass what was deliberately left out as the complete_goal leftOut parameter — it renders under Remaining and is omitted when absent. When the work spans distinct areas (subsystems, screens, phases), group the findings with the complete_goal findingGroups parameter — one group per area, each finding leads with the primary user-visible outcome, then uses ` — ` for concrete evidence/reason (repo-relative `path:line`, count/version, or explicit reason; never invent proof). `Lead:` is a semantic role, not a UI label: do not prefix findings with `Lead:`, `Tests:`, or `Verdict:`. Chat renders readable nested area sections under What Changed; the archive retains the detailed evidence table for 4+ groups. Add per-finding test proof with the parallel `tests` array (tests[i] proves findings[i]) only when it demonstrates behavior; it is detailed archive support, not default chat copy. When the work ran named quality gates (unit, typecheck, E2E, build, invariants), pass the complete_goal gateRows inventory — one row per gate with scope and outcome notes, plus the repro `command` when one exists. The archive retains the Quality Gate | Command | Scope | Status | Notes table with mechanically derived statuses (PASS only when the notes say pass with zero failures); chat shows these only after an explicit `showVerification: true` request, as one aggregate sentence. Single-area work omits findingGroups — the flat human change render stays the fallback; work with no gate inventory omits gateRows — the archive uses its mechanical Tests row while chat omits technical Tests by default. Verification is supporting evidence, never the main body of the change story.
