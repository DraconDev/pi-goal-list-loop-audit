...
test("v0.34.22: complete_goal returns while a detached auditor finishes and archives approval", async () => {
  ...
  try {
    ...
    assert.ok(MAIN_SM.entries.some(e => e.content?.startsWith("## Done — The detached completion path is covered")), "persisted briefing leads with outcome");
...
test("v0.34.91: detached approval notify carries the agent's completion recap, not process boilerplate", async () => {
  ...
  try {
    ...
    assert.ok(recapNotifs.some((n: { message: string }) => n.message.includes("### What Changed") && n.message.includes("**extensions/goal-loop-display.ts and tests/behavioral-orchestrator.test.ts.**")), "the recap arrives sectioned around concrete change facts");
