...
test("fixture: repeated real continuation payloads grow context linearly and are isolated", () => {
  ...
  // Audit 2026-09-13: continuation list-tool consent guidance (pasted-list
  // wording rule, list_activate selection guard, Confirm-gate restatement)
  // — +348 bytes per payload (+346 chars; +348 serialized with the two
  // straight-quote escapes). Audit 2026-09-15: parity-accurate claim
  // guidance (uncapped values, Command column, scoped machine-path ban)
  // — +262 bytes per payload (+260 chars, +1 em dash; +262 serialized).
  // Audit 2026-09-15 (DECIDED: keep one-action Next): parity language
  // scoped to findings/values — +51 bytes per payload (+49 chars, +1 em
  // dash; +51 serialized). Agent resume (Screenshot_20260916_090307):
  // paused-goal agent self-resume guidance — +377 bytes per payload
  // (+375 chars, +1 em dash; +377 serialized). Linear growth is
  // preserved (see exact multiples below).
  // Linear
  ...
  assert.equal(payload.length, 25_275);
  assert.equal(new TextEncoder().encode(payload).byteLength, 25_387);
  assert.deepEqual(one, {
    messageCount: 3,
    serializedBytes: 25_920,
    textChars: 25_327,
    estimatedTokens: 6_332,
    gllaMessageCount: 1,
    gllaSerializedBytes: 25_737,
    gllaTextChars: 25_275,
    gllaEstimatedTokens: 6_319,
  ...
  assert.deepEqual(twelve, {
    messageCount: 14,
    serializedBytes: 309_027,
    textChars: 303_352,
    estimatedTokens: 75_838,
    gllaMessageCount: 12,
    gllaSerializedBytes: 308_844,
    gllaTextChars: 303_300,
    gllaEstimatedTokens: 75_825,
    uniqueGllaPayloadCount: 1,
    repeatedGllaPayloadCount: 11,
    repeatedGllaSerializedBytes: 283_107,
    failedErrorOnlyCount: 0,
    unserializableMessageCount: 0,
    provider: {
      sampleCount: 12,
...
test("reported checkpoint shape stays pinned across all probe sizes", () => {
  ...
  assert.deepEqual(rows, [
    ...
    },
    {
      continuations: 5,
      messageCount: 7,
      serializedBytes: 128_868,
      textChars: 126_427,
      estimatedTokens: 31_607,
      gllaMessageCount: 5,
      gllaSerializedBytes: 128_685,
      gllaTextChars: 126_375,
      gllaEstimatedTokens: 31_594,
      repeatedGllaPayloadCount: 4,
      repeatedGllaSerializedBytes: 102_948,
      provider: {
        sampleCount: 5,
        inputTokens: 60_000,
        outputTokens: 510,
    ...
    },
    {
      continuations: 25,
      messageCount: 27,
      serializedBytes: 643_608,
      textChars: 631_927,
      estimatedTokens: 157_982,
      gllaMessageCount: 25,
      gllaSerializedBytes: 643_425,
      gllaTextChars: 631_875,
      gllaEstimatedTokens: 157_969,
      repeatedGllaPayloadCount: 24,
      repeatedGllaSerializedBytes: 617_688,
      provider: {
        sampleCount: 25,
        inputTokens: 800_000,
        outputTokens: 2_800,
