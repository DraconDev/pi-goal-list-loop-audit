...
test("v0.28.26: quota-blocked audits store the claim + the retry re-runs the AUDITOR directly (no agent turn)", () => {
  ...
  assert.match(SRC, /const archived = archiveCurrentGoal\(liveCtx, "complete", `auditor \$\{result\.model\} approved \(\$\{origin\}\)`, \{\}, \{ findingGroups: claim\.findingGroups, gateRows: claim\.gateRows, priorCompletionSummary: claim\.priorCompletionSummary \}\)/, "v0.38.52: the audited claim's finding groups + gate rows ride into the archive");
