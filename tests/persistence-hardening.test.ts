...
test("E1: archive destination is exclusive and cannot clobber a winner", () => {
  ...
  assert.match(GOAL, /buildRichArchiveSection\(terminalGoal, status, archivePath, opts\?\.findingGroups, opts\?\.gateRows, opts\?\.priorCompletionSummary\)/, "archive carries the rich human layer over the machine record");
