 ...
 * schema rejects past its own maxLength, so a 500-char sanitizer bound would
 * silently eat what the schema now accepts. Clause-aware cut, never mid-word. */
export const MAX_GROUP_FINDING_CHARS = 10000;
/** v0.38.52: per-finding test-result lines share the finding budget. */
export const MAX_GROUP_TESTS_CHARS = 10000;
...
export function sanitizeFindingGroups(value: unknown): FindingGroup[] | undefined {
  ...
  for (const entry of value) {
    ...
    for (const finding of raw.findings) {
      ...
      const text = clipSummaryValue(finding.trim(), MAX_GROUP_FINDING_CHARS);
    ...
    if (Array.isArray(raw.tests)) {
      ...
      for (const entry of raw.tests) {
        ...
        const text = clipSummaryValue(entry.trim(), MAX_GROUP_TESTS_CHARS);
...
export type GoalRoute =
  ...
  | { kind: "sub"; name: "status" | "pause" | "resume" | "cancel" | "decide" | "verify" | "audit" | "tweak" | "archive" | "start" | "plan" | "timeline"; rest: string };
...
const GOAL_ARG_SUBS = new Set(["audit", "tweak", "archive", "start", "plan", "timeline"]);
...
export function routeGoalArgs(raw: string): GoalRoute {
  ...
  if (GOAL_ARG_SUBS.has(first)) {
    return { kind: "sub", name: first as "audit" | "tweak" | "archive" | "start" | "plan" | "timeline", rest };
  }
...
export function buildSeedGrillMessage(tmpl: string, seed: string, tool: string): string {
  return `${tmpl}\n\n${LONG_RUNNING_JUDGMENT_POLICY}\n\nThe user's initial objective (verbatim): ${seed}\n\nGRILL THEM ABOUT THIS SEED BEFORE PROPOSING. ${tool} is BLOCKED until the user has replied to at least one of your questions — proposing without interviewing returns an error.\n\nHow to grill:\n- Ask 2-4 sharp, seed-specific questions in ONE batched ask_user_question call per phase when several unknowns exist — about THIS objective, not generic filler. Each question ships with a recommended default the user can accept with "yes" (one picker, 2-4 concrete options per question). If only one unknown remains, one focused question is fine. For structured choices, you MUST use ask_user_question when available; do not substitute a numbered prose questionnaire. Plain conversation is reserved for genuinely free-form questions or tool unavailability. State the drafting roadmap first, batch independent questions within the current phase, and ask dependent questions only after the earlier answers. After an answer, either present the next question or propose the concrete contract; never end a turn merely promising questions. When a question has actually been presented, wait for the user — do not autonomously repeat it.\n- Probe what matters across the drafting phases: what "done" concretely looks like (checkable evidence — files, commands, behaviors), scope boundaries (what is explicitly OUT), constraints (what must not change), and priorities when the seed bundles several wishes. Resolve these decisions during drafting, before execution. Batch independent questions, but never guess an earlier answer to squeeze dependent decisions into one upfront batch.\n- A non-answer ("not sure", "none", "whatever") is a trigger to offer 2-3 concrete options to pick from — never silently proceed on a non-answer.\n- Do targeted read-only research first when it makes your questions sharper (repo layout, existing docs).\n- Do NOT activate the raw seed. Do NOT implement anything. When the contract is concrete, call ${tool}.`;
}
