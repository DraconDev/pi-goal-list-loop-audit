...
class JsonRpcProcess {
  ...
  emit(event) {
    ...
    if (process.env.GLLA_LIVE_DEBUG === "1") {
      if (event?.type === "compaction_end") globalThis.__gllaCompactionEnd = { aborted: event.aborted === true, willRetry: event.willRetry === true, error: event.errorClass ?? "none", summaryChars: event.result?.summaryChars ?? 0 };
    }
