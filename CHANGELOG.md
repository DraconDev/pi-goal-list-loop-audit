...
not terminal chat. Empty receipt-only groups disappear without losing test
proof alignment; failures, skips, partial checks and scope limitations remain
visible. Substantive ledger fixes and useful implementation references are
preserved. Screenshot-derived regressions cover the Neonbreak Process entry.

Reconciled note.md to verified publication and removed an accidentally
repeated historical changelog tail; existing release history is unchanged.

## 0.38.58 — Evidence-based status and clean citations (2026-09-17)

### Truthful monitoring labels

Queued goals no longer display "MONITORING · next check" from objective keywords; with no runtime monitoring producer, queued turns stay queued while working, auditing and recovery evidence keep their truthful labels (field auditor repro).

### Complete citation extraction

Multi-line `path.ts:672,2760` references are removed whole from chat, preserving them in the archive — no more `(,2760)` husks (Deathrun screenshot).
